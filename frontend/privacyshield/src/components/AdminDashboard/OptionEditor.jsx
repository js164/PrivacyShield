            ...
            {option.suggestion && (
                <div className="px-4 pb-3">
                    <div className={`text-xs italic text-slate-500 bg-slate-100 p-2 rounded-md`}>
                        <span className={`font-bold not-italic px-2 py-0.5 rounded-full text-xs mr-2 ${suggestionCategoryColor[option.suggestion_category]}`}>{SUGGESTION_CATEGORIES[option.suggestion_category]}</span>
                        {option.suggestion}
                    </div>
                </div>
            )}
                    ...
                    {option.scores.map((value, key) => (
                        <div key={key} className="flex items-center justify-between bg-slate-100 p-2 rounded-md">
                            <div className="relative group flex items-center gap-1.5">
                                <span className="text-sm font-medium text-slate-600 cursor-pointer">{value.code}:</span>
